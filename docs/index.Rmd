...
## Value of MIR median and min considering every disease
```{r}
...
w %>% 
  ...
  # filter(VBD_Agent_subtype == "ASFV") %>% 
  group_by(VBD_AS_Full) %>% 
  summarise(M=median(MIR), min(MIR)) 
# %>% 
#   arrange(m)

```
## MIR considerig disease and vector group
```{r}
datatable(w %>% 
  filter(MIR > 0) %>% 
  filter(VBD_Agent_subtype %in% di) %>%  
  group_by(VBD_AS_Full, VectorGroup) %>% 
  summarise(Min=round(min(MIR)), Median=round(median(MIR),2), Max=round(max(MIR),2)))

```

## Boxplot of MIR
There is the problem with the selecting filter applied on the document, considering feeding status and life stage
```{r}
w %>% 
  filter(MIR > 0) %>% 
  filter(VBD_Agent_subtype %in% di) %>% 
  ggplot(aes(MIR, color=VBD_AS_Full))+
  geom_boxplot()
